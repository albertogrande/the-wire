...
| **The Daily Dive** | A shorter (~1,200–1,800w) technical dive under a rotating columnist byline (see `AUTHORS.md`). Evergreen "how X works" by default; today's news only when it earns a durable explainer. Topics from the scout's signals or `topics/backlog.md`. `reports/deep-dives/` | Tue–Sat; **bakeoff** (all three columnists, one topic) Sundays |
